...
function generateCard(id, productUrl, img, title, subtitle, description) {
    let cardForm = `
                <form class="card card-hovered d-flex h-100 flex-column justify-content-between" id="${id}" name="${id}" method="POST" action="../components/scripts/update.php?id=${id}" >
...
$('#deleteModal').on('shown.bs.modal', function (e) {
    ...
    $('#delete-form').attr('action', `../components/scripts/delete.php?id=${id}`);
})
